...
ggplot(data = final_join, aes(x = agriculture, y = forest_cover)) + geom_point(aes(color=year, size=malaria))+
  geom_smooth(method="lm")+ scale_color_gradient(low="yellow", high="red")+ ggtitle("Comparison of Land Used for Agriculture vs Covered in Forest")+ xlab("Percent of Land Used for Agriculture")+ ylab("Percent of Land Covered in Forest")+ scale_y_continuous(breaks=seq(0, 100, 10))+ scale_x_continuous(breaks=seq(0, 100, 10))+ theme_gray()
...
amazon <- final_join %>% filter(country %in% c("Brazil","Columbia", "Peru", "Bolivia", "Venezuela", "Guyana", "Suriname"))
 ...
 ggplot(amazon, aes(x = country, y = malaria, fill=country))+
  geom_bar(stat="summary",fun=mean)+
  geom_errorbar(stat="summary", fun.data=mean_se, width=.5)+ggtitle("Malaria Cases per 100,000 people in Amazonian Countries")+ xlab("Country")+ ylab("Malaria Cases (per 100,000 people)")+ theme(legend.position="none")
...
ggplot(data = deforest, aes(x= year, y= livestock))+ geom_point(aes(color=country), size=2)+ geom_smooth(aes(color=country), method="lm", se=F, size=.5)+ theme_gray()+ ggtitle("Livestock Production in Countries with High Deforestation Rates")+ ylab("Livestock Production Index")+ xlab("Year")+ scale_x_continuous(breaks=seq(1995, 2005, 1))+ theme_bw()+ geom_point(stat="summary", size=6)+ geom_smooth(method="lm", color="black")
